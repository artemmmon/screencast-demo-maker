---
name: demo-scenario
description: Writes the shooting script for a demo/screencast video — scenes with what to show, what to do and what to say — and the cue list that narration is generated from, with every quoted number checked against the running app. Use when the user asks for a demo script, a video scenario, a walkthrough plan for a feature, PR or homework, or says "demo script", "сценарій демо", "план демо-відео". Also use before filming with demo-film, which consumes the files this produces. Not for filming (demo-film) or first-time project setup (demo-setup).
---

# Demo scenario

Produces the first two of the four files that filming needs, in the video's demo folder
(`<demoDir>`, named in the project's `*-demo` skill; `demo/<name>/` when there is none):

- `scenario.md` — human-readable: scenes, what is on screen, what is said.
- `cues.json` — a bare JSON array `[{ "id": "s2-04", "scene": "s2", "text": "…" }]`.

The other two are not yours: `config.json` (surfaces, voice, display, URLs) comes from
`demo-setup`, and `scenes.mjs` (the code that drives the screen) from `demo-film`. You only
set `config.order` to the scene ids you wrote. A scenario is useful on its own, including
to a person filming by hand.

A project's own demo skill (`*-demo`), if it has one, names a worked example of all four.

## 1. Intake

First read the project's `*-demo` skill and `<demoDir>/config.json`. If neither exists,
say so and offer `demo-setup` first — it asks about surfaces, URLs, voice and display
once for the whole project. The user may decline; then continue here.

Then ask **up front, in one AskUserQuestion call**, every item below that the files above
do not already answer — never start writing scenes on a guess. When you cannot ask —
running unattended, or told not to — assume the defaults below, and list every assumption
at the top of `scenario.md` under `## Assumptions` instead of stalling.

- **Source of truth** — grading criteria, a spec, a PR, a diff, a feature list.
  Every criterion must end up covered by a scene, so read it first.
- **Where it goes** — `<demoDir>` for this video.
- **Target length** — default 4–6 minutes. Narration runs ~14 characters/second
  (`config.tts.charsPerSecond`; measure it once for a new voice or language),
...
- **Language** of narration, and who the audience is (a reviewer who knows the
  codebase needs no product intro). Default: `config.tts.language`, else the user's
  spoken language. The files around the narration — headings, notes, coverage tables,
  comments — stay English, so a Ukrainian voice-over still lives in an English
  `scenario.md`.
- **Mutating controls** — `config.neverClick` is never planned into a scene. Anything
  else that changes data needs the user's yes and a pre-roll that restores the state.
...
import("playwright").then(async ({ chromium }) => {
  ...
  await p.goto("<config.web.baseUrl>/…", { waitUntil: "networkidle" });
...
- **Line anchors** (editor surface only) — `grep -n` every file:line the video jumps to; put them in the
...
Set `config.order` to the scene ids in playback order. Tell the user: the two file paths,
the estimated length (`chars / tts.charsPerSecond` seconds, 14 by default), and what the
reality check changed or could not verify.
...
needs the app running and whatever the doctor still flags
(`node ${CLAUDE_SKILL_DIR}/../demo-film/scripts/doctor.mjs <demoDir>`).
